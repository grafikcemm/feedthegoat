  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-6 border-t border-ftg-border-subtle bg-ftg-surface/30">
          {allPackagesCompleted ? (
            <button
              onClick={handleFinalize}
              disabled={isPending}
              className="w-full py-4 bg-ftg-success hover:bg-green-500 text-white font-mono text-xs font-bold tracking-[0.2em] rounded-ftg-card transition-all shadow-xl shadow-green-900/40 active:scale-95 disabled:opacity-50 animate-in fade-in zoom-in-95"
            >
              ✓ TAMAMLANDI — KAPAT
            </button>
          ) : (
            <div className="flex justify-center">
              <p className="font-mono text-[9px] text-ftg-text-mute italic">Antigravity Skincare Tracking System v1.1</p>
            </div>
          )}
        </div>
      </div>
